...
export default async function create() {
    if (existsFile("info") && existsFile("readme")) {
        console.log("⚠️ README.md e docs/info.json já existem. Não sobrescrevendo.");
    }
    else {
...
async function askStacks() {
    ...
    const stacks = new Set(stacksInput
        .split(",")
        .map((s) => s.trim())
        .filter(Boolean));
    return [...stacks];
...
async function askImages() {
    ...
    while (addMore) {
        const url = await input({
            message: "URL/PATH:",
            validate: (v) => v.startsWith("http") || v.startsWith("docs/") || "URL inválida",
        });
        const more = await confirm({
